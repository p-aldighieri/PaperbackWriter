...
- **Use case ≠ question (WISDOM §11) — handle it autonomously.** If the target's `brief.md`/`proofs/` encode no economic question — the math is result→certificate-shaped and the "application" could be swapped out without changing an assumption or theorem — do not decorate it with motivation, and do not stop to ask the user. Run the `framing-writer` agent (Phase 0 of `draft-paper`): it derives the question by working backwards from the headline theorem, rewrites `brief.md` (original backed up to `brief.original.md`), and logs the derivation in `<target>/notes/derived-framing.md`. Framing is *derived, never invented*: every element must trace to what the theorems actually establish.
