...
**The contract.** An economic theory paper runs *question → minimal model → insight*: an economic question stated first, a model presented as the minimal vehicle for that question, theorems that answer it in economic terms. A math or CS paper runs *result → certificate → robustness*. Drafts whose math was produced first default to the second shape, and a theorist pattern-matches the shape instantly. Rewording cannot fix this; the paper's spine must be rebuilt around the question. When the incoming brief supplies no question, the question is *derived from the theorem itself* — work backwards: what economic question is this result the answer to? — and the derivation recorded (`notes/derived-framing.md`; the `framing-writer` agent owns this). Derivation is legitimate framing work; what is forbidden is *decoration* — wallpapering a question-less result with motivation (see "use case ≠ question" below).
...
**The main result must exist as an economics sentence.** Somewhere before its formal statement, the headline result must appear as one sentence a PhD economist outside the subfield could repeat at lunch — e.g., "receiver control reshapes the value of persuasion but never the form of the optimal experiment." If no such sentence is on hand, constructing it is the first framing task (work backwards from the theorem; record it in `notes/derived-framing.md`); nothing downstream — abstract, intro, motivation — should be drafted before it exists. The constructed sentence restates exactly what is proven, never more.
...
**Assumptions are standard or defended — once, in place.** Every non-standard assumption gets one sentence of defense where it is introduced (§7, item 6) — not five paragraphs, not zero, and never a trailing "honest scope" / "knife-edge cases" inventory. A collected caveats dump tells the referee the model was built backwards from the theorem. If an assumption exists only to make the proof go through, give it the honest one-sentence acknowledgement in place ("this isolates the effect of X" / "this is the price of tractability, relaxed in §n" — whichever is true) and log it in the framing notes' assumption ledger so every writer places it consistently.
