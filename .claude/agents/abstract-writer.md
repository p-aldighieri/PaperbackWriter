...
- **Lead with the question and its answer in economics words.** The first two sentences state the economic question and the main result as plain English a PhD economist outside the subfield can repeat — no symbols, no model machinery. If no such sentence exists in `brief.md`, check `<target>/notes/derived-framing.md` (Phase 0 usually wrote it); if that's missing too, construct the sentence yourself by working backwards from the headline theorem (WISDOM §11) and note `[FRAMING-DERIVED]` in the audit line. The constructed sentence restates exactly what is proven — nothing stronger.
